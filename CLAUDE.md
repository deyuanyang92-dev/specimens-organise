...
## GLM-5.1 terminal helper

`glm51_chat.py` (+ `glm` wrapper script) is a standalone CLI for chatting with GLM-5.1 — unrelated to the specimen app. Needs `zai-sdk` (in `requirements.txt`) and `ZAI_API_KEY` env var. Thinking mode on by default; `--show-reasoning` prints reasoning. Run `python3 glm51_chat.py "问题"` or `bash glm "问题"`.
